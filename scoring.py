...
Usage examples:
    python scoring.py score_all             # score all IOCs in DB
    python scoring.py top 50                # show top 50 prioritized IOCs
    python scoring.py score_one <ioc_value> # score a single IOC (value must match DB)
"""
...
def compute_score(enrichment: Dict[str, Any], meta: Optional[Dict[str,Any]] = None) -> int:
    ...
    try:
        ...
        if otx and isinstance(otx, dict):
            ...
            score += pulses * WEIGHTS["otx_pulse"]
    except Exception:
        pass
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python scoring.py score_all | top <N> | score_one <IOC_VALUE>")
        sys.exit(1)
    ...
    if cmd == "score_all":
        ...
        for r in rows:
            ...
            try:
                ...
                pass
            print(f"{r['value']:<40} {r['ioc_type']:<6} SCORE={r['score']:3} UPDATED={r['score_updated_at']} META_FEEDS={len(meta.get('sources',[]) if isinstance(meta, dict) else 0)}")
